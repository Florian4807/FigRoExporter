...
function init() {
    return __awaiter(this, void 0, void 0, function* () {
        // Read cached state
        const cached = yield figma.clientStorage.getAsync('roblox_ui_state');
        if (cached) {
            state = Object.assign(Object.assign({}, state), cached);
        }
        scanForCanvas();
...
figma.ui.onmessage = msg => {
    if (msg.type === 'create-canvas') {
        ...
        const cvs = getCanvas();
        if (!cvs) {
            figma.notify("Need a Canvas first.");
            return;
        }
        ...
        if (sel && sel.type === 'FRAME' && sel.getPluginData('isRobloxFrame')) {
            ...
            if (!cvs)
                return;
            if (msg.dir === 'H' || msg.dir === 'HV')
                sel.x = (state.canvasW - sel.width) / 2;
            if (msg.dir === 'V' || msg.dir === 'HV')
                sel.y = (state.canvasH - sel.height) / 2;
        }
    }
...
function calculateAndApplyAnchor(node, w, h, ax, ay, cw, ch) {
    if (ax === 'left')
        node.x = 0;
    else if (ax === 'center')
        node.x = (cw - w) / 2;
